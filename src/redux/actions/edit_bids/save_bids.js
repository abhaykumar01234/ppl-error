...
const saveBids = async (changedSizeData, changedSegmentData, dispatch) => {
  ...
  try {
    ...
    isSaveBidSuccess =
      responseList.length > 0 ? responseList.every(response => response.ok) : false;
  } catch (err) {
...
export default productId => async (dispatch, getState) => {
  const { changedBids } = getState();
  const { changedSizeData, changedSegmentData } = mapChangedBids(changedBids);

  const isSaveSuccess = await saveBids(changedSizeData, changedSegmentData, dispatch);
