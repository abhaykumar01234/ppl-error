...
export default (productIdList, refresh = false) =>
  async (dispatch, getState) => {
    const { sizes } = getState();
    if (!refresh && sizes[productIdList[0]]) return null;

    try {
      const sizesPromiseList = productIdList.map(productId =>
        authFetch(`/products/${productId}/sizes`)
          .then(response => {
