...
describe('Edit Bids Page', function () {
  ...
  context('Auction Bids Table', function () {
    it('should display initial content', function () {
      ...
      const placeholderText = 'Need help? Use this tool to find and set optimal bids. Select any bid to view the estimated position and cost per lead at that bid amount. Questions?';
      cy.get('[class*=auction-bids-placeholder-wrapper]').should('be.visible').contains(placeholderText);
    ...
    context('when representative email is available', function () {
      it('should display contact modal with representative email', function () {
        ...
        cy.get('[data-cy=contact-link]').should('be.visible').then(($link) => {
          $link.trigger('click');
          cy.get('[data-cy=advisor-modal]').contains('Contact Your Advisor').should('be.visible');
          cy.get('[data-cy=advisor-modal]').and('contain', this.vendorFixtureData.representative.email);
    ...
    it('should show vendor position', function () {
      ...
      cy.get('[data-cy=auction-bids-table]').children().then(($BidRows) => {
        const vendorCurrentBidRowIndex = this.auctionBidsData.findIndex(bid => Boolean(bid.isUserPosition));

        cy.get($BidRows[vendorCurrentBidRowIndex]).children(vendorCurrentBidRowIndex).last().then((lastColumn) => {
          cy.get(lastColumn).find('div')
            .should('be.visible')
            .and('have.attr', 'class')
    ...
    it('should fill the default bid value when auction bid value is selected', function () {
      ...
      cy.get('@bidRowInput').invoke('val').then((value) => { oldBidValue = value; });
      ...
      cy.get('@firstAuctionBidRow')
        ...
        .then(() => {
          ...
          cy.get('[data-cy="bid-row"]:nth(0)').find('div')
            .should('be.visible')
            .and('have.attr', 'class')
            .and('contain', 'icon-filled-circle');
