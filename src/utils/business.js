...
export const getNewMaxCostFromAuctionBids = (auctionBids, bidAmount) => {
  let updatedBid = auctionBids.reduce((total, curr) => {
    if (curr <= bidAmount) total = Math.max(curr, total);
    return total;
  }, 0);
  updatedBid = !updatedBid ? bidAmount : updatedBid + 5 > bidAmount ? updatedBid : updatedBid + 5;
  return updatedBid;
};
...
export const isBidValid = (bid, baseAmt) =>
  bid !== 0 && bid % PPL_BID_INCREMENT === 0 && bid > baseAmt;
